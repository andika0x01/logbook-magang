  ...
  const modalContent = (
    <AnimatePresence>
      {previewUrl && (
        ...
        >
          <div style={{ height: '64px', padding: '0 24px', display: 'flex', justifyContent: 'space-between', alignItems: 'center', flexShrink: 0, backgroundColor: '#000', borderBottom: '1px solid #27272a' }}>
            ...
            <button 
              type="button" onClick={() => setPreviewUrl(null)}
              className="mission-btn py-1.5 px-6 border-white text-white hover:bg-white hover:text-black"
            >
              Close Viewer [X]
            </button>
          </div>
        ...
        <div className="space-y-10">
          <form method="post" encType="multipart/form-data" className="space-y-10">
            ...
            <div className="console-panel p-1">
              ...
              <textarea 
                name="content" required defaultValue={log?.content || ""}
                rows={12}
                className="w-full bg-black p-6 text-white focus:outline-none text-[15px] leading-relaxed font-sans placeholder:text-white/10"
                ...
                <div className="flex flex-col md:flex-row items-center gap-6">
                  ...
                  <div className="flex-none text-center md:text-left">
                    <p className="text-[10px] text-terminal-gray/60 uppercase tracking-widest font-bold leading-relaxed">
                      Auto-Sync to Google Drive established.<br/>
                ...
                {attachments.length === 0 ? (
                  ...
                  <div className="grid grid-cols-2 md:grid-cols-3 lg:grid-cols-4 gap-4">
                    {attachments.map(att => {
                      ...
                      return (
                        <div key={att.id} className="console-panel border-white/5 group hover:border-white/20 transition-all cursor-pointer relative overflow-hidden aspect-square" onClick={() => setPreviewUrl(att.url.replace(/\/view.*$/, '/preview'))}>
                          ...
                          <div className="absolute top-2 right-2 z-20">
                            <button 
                              type="submit" name="delete_media_id" value={att.id} 
